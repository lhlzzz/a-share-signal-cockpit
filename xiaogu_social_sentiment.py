#!/usr/bin/env python3
"""Eastmoney social evidence collector: direct public JSON API only."""
import argparse
import html
import json
import os
...
def _scrape_eastmoney_guba_api(symbol: str, *, page_size: int = 30) -> Dict[str, Any]:
    """Primary path: gbapi article list JSON."""
...
def scrape_eastmoney_guba(symbol: str) -> Dict[str, Any]:
    """Collect guba titles through the direct public JSON API."""
    code = str(symbol or "").zfill(6)
    try:
        return _scrape_eastmoney_guba_api(code)
    except Exception as exc:
        return {
            "error": f"eastmoney_guba_api_failed:{type(exc).__name__}:{exc}",
            "symbol": code,
        }
