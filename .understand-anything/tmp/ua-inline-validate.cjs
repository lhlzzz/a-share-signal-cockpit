...
try {
  ...
  const fileNodes = graph.nodes.filter(n => fileTypes.has(n.type)).map(n => n.id);
  ...
  graph.layers.forEach((layer, i) => {
    for (const key of ['id','name','description','nodeIds']) if (typeof layer[key] !== (key === 'nodeIds' ? 'object' : 'string')) issues.push(`Layer[${i}] missing ${key}`);
    (layer.nodeIds || []).forEach(id => { if (!nodeIds.has(id)) issues.push(`Layer '${layer.id}' refs missing node '${id}'`); if (assigned.has(id)) issues.push(`Node '${id}' appears in multiple layers`); assigned.set(id, layer.id); });
  });
  fileNodes.forEach(id => { if (!assigned.has(id)) issues.push(`File node '${id}' not in any layer`); });
  graph.tour.forEach((step, i) => {
    for (const key of ['order','title','description','nodeIds']) if (step[key] === undefined) issues.push(`Tour[${i}] missing ${key}`);
    (step.nodeIds || []).forEach(id => { if (!nodeIds.has(id)) issues.push(`Tour[${i}] refs missing node '${id}'`); });
  });
